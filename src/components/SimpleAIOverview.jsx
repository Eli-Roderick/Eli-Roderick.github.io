...
function processContent(html) {
  ...
  const createImageRow = (imageMatches) => {
    ...
    let row = `<div class="image-row-container" style="clear: both; margin: 0.25rem 0;">
  ...
  processed = processed.replace(/\[([^\]]+)\]/g, (match, url) => {
    // Check if it's an image URL
    if (/https?:\/\/[^\s]+\.(?:jpg|jpeg|png|gif|webp|svg|bmp)(?:\?.*)?$/i.test(url)) {
      return `<div style="clear: both; margin: 0.25rem 0;"><img src="${url}" alt="User provided image" style="max-width: 200px; height: auto; border-radius: 0.5rem; display: block;" /></div>`
    }
